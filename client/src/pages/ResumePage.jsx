...
import InputPanel   from './Resume/ResumeInputpanel.jsx'
import ResultsPanel from './Resume/ResumeResultspanel.jsx'
import Navbar       from './Resume/ResumeNavbar.jsx'
import Footer       from './Resume/ResumeFooter.jsx'
  ...
  async function handleEvaluate({ file, text, jdText, mode }) {
    ...
    try {
      ...
      if (mode === 'file') {
        ...
        if (studentId) form.append('student_id', studentId)   // ← sends student_id to backend
        response = await fetch('http://localhost:8000/api/resume/evaluate', { method: 'POST', body: form })
      } else {
        response = await fetch('http://localhost:8000/api/resume/evaluate-text', {
          ...
          body:    JSON.stringify({
            ...
            student_id:  studentId || null,                    // ← sends student_id to backend
          }),
        })
      }
      if (!response.ok) {
